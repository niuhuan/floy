use crate::commons::utils::join_paths;
use crate::domain::defines::get_database_dir;
...
pub(crate) async fn connect_db(path: &str) -> DatabaseConnection {
    debug!("CONNECT TO DB : {}", path);
    let path = join_paths(vec![get_database_dir().as_str(), path]);
    debug!("DB PATH : {}", path);
